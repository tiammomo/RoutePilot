...
- legacy plan preview results now also come back as an explicit `SupervisorPlanPreview` contract before [agent/travel_agent/runtime/agent_runtime.py](/D:/moyuan/moyuan-travel-agent/agent/travel_agent/runtime/agent_runtime.py) enriches them with artifact/subagent metadata, which narrows the preview seam to one typed compatibility payload instead of a loose dict
- legacy tool-health diagnostics now also come back as explicit `SupervisorToolHealthDiagnostics / SupervisorToolHealthEntry` contracts before [agent/travel_agent/runtime/agent_runtime.py](/D:/moyuan/moyuan-travel-agent/agent/travel_agent/runtime/agent_runtime.py) enriches them with skills/subagents metadata, which narrows the diagnostics seam to one typed compatibility payload instead of a loose dict
