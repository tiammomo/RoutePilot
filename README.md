...
- `frontend/src/components/travel-plan-toolkit/shared/artifact.ts` 负责 artifact-first 的 destinations / budget / verification 摘要与分享内容构造；当 `TravelPlanToolkit` 已拿到结构化 artifact 时，overview 和分享短链都会优先消费 artifact，而不是继续直接依赖原始长文本
